...
def parse_arguments():
    ...
    parser.add_argument("-P", "--pins", default = "21,20,16,12,18,7,8,25,24,23", metavar="numbers", help="determines the number of piezo speakers too, default is \"21,20,16,12,18,7,8,25,24,23\"")
...
def convert_time(data, speed):
    ...
    for i in data:
        ...
        if i[2] == "Note_on_c" and i[5] != "0" and i[3] != "9":
            new_data.append([length, True, int(i[4]), int(i[3])])
            note_count += 1
        if ((i[2] == "Note_on_c" and i[5] == "0") or i[2] == "Note_off_c") and i[3] != "9":
            new_data.append([length, False, int(i[4]), int(i[3])])
            ...
            if i[2] in bad_offs:
                ...
                for jndex, j in enumerate(sounding_notes):
                    if j[1] == i[2]:
                        ...
                        new_data.append([i[0], shifted_note[0], 0])
                        break
...
def play(data):
    prevtime = 0
    for i in data:
        time.sleep(i[0] - prevtime)
        prevtime = i[0]
        if i[1] == MOTOR_PIN_1:
            wiringpi.softToneWrite(i[1], i[2]*2)
            wiringpi.softToneWrite(DIR_PIN_1, i[2])
#            pass
        elif i[1] == MOTOR_PIN_2:
            wiringpi.softToneWrite(i[1], i[2]*2)
            wiringpi.softToneWrite(DIR_PIN_2, i[2])
        elif i[1] == MOTOR_PIN_3:
            wiringpi.softToneWrite(i[1], i[2]*2)
            wiringpi.softToneWrite(DIR_PIN_3, i[2])
        else:
...
def main():
    ...
    [wiringpi.softToneCreate(i) for i in pins + [DIR_PIN_1, MOTOR_PIN_1, DIR_PIN_2, MOTOR_PIN_2, DIR_PIN_3, MOTOR_PIN_3]]
...
#        print(data)
#        print('\n\n')
        track1data = [[i[0], i[1], i[2]+24] for i in data if i[3] == 2]
        trackodata = [[i[0], i[1], i[2]-24] for i in data if i[3] != 2]
#        print(track1data)
#        print(trackodata)
        track1new, boc1 = restrict_number_of_notes(track1data, pins, args["shiftpitch"])
        trackonew, boc2 = restrict_number_of_notes(trackodata, [MOTOR_PIN_1, MOTOR_PIN_2, MOTOR_PIN_3], args["shiftpitch"])

        stats["bad_offs_count"] = boc1 + boc2
        data = track1new + trackonew
